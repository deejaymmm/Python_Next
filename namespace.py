def get_parent(space, variable):
  cur_space = namespaces[space]
  cur_variables = cur_space['variables']
  cur_parent = cur_space['parent']
  if variable in cur_variables:
    return space
  elif cur_parent == None:
    return None
  else:
    return get_parent(cur_parent, variable)
...
for cur_com in range(num_com):
  command, param1, param2 = input().split()
  if command == 'create':
    namespaces[param1] = {'variables': [], 'parent': param2}
  elif command == 'add':
    cur_space = namespaces[param1]
    cur_variables = cur_space['variables']
    cur_parent = cur_space['parent']
    cur_variables.append(param2)
    namespaces[param1] = {'variables': cur_variables, 'parent': cur_parent}
  elif command == 'get':
    print(get_parent(param1, param2))
...
Аналогично, результатом запроса get bar b будет являться foo, 
